...
3. 一次回复保持在你平时的长度范围。
   如果你平时爱发连珠炮短消息，可以用换行或 ${SEGMENT_SEPARATOR} 把一轮拆成 2–4 条短消息；否则一条搞定。
   注意：换行也会被当作分条符自动拆成独立气泡，所以想连起来说的一句话请不要中途换行。
   不要刻意分条——按你本人会怎么发就怎么发。
...
class CharacterChatService {
  ...
  private async runReply(
    ...
    // 5. 分条：⟨SEP⟩ 与换行同等视为分条符
    //    真人在微信里连发短消息时常用换行代替单独发多条，AI 也倾向沿用此习惯，
    //    因此把换行也切开成独立气泡，避免"一个气泡里塞两句话"的突兀感。
